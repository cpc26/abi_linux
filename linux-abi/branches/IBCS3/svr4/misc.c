...
abi_fork(struct pt_regs *regs)
{
	...
	_FLG(regs) &= ~1;
/*	retval = do_fork(SIGCHLD, regs->esp, regs, 0, NULL, NULL);*/
...
abi_wait(struct pt_regs * regs)
{
	...
	if ((_FLG(regs) & MAGIC_WAITPID_FLAG) == MAGIC_WAITPID_FLAG) {
		get_user(pid, ((u_long *)_SP(regs))+1);
		get_user(loc, ((u_long *)_SP(regs))+2);
		get_user(opt, ((u_long *)_SP(regs))+3);
		...
		get_user(loc, ((u_long *)_SP(regs))+1);
...
abi_exec(struct pt_regs *regs)
{
	...
	ulptr = (u_int *)(&pgm);
	if (get_user(*ulptr, ((u_int *)_SP(regs))+1))
		return -EFAULT;
	ulptr = (u_int *)(&argv);
	if (get_user(*ulptr, ((u_int *)_SP(regs))+2))
		return -EFAULT;
	ulptr = (u_int *)(&envp);
	if (get_user(*ulptr, ((u_int *)_SP(regs))+3))
		return -EFAULT;
...
#endif
	obx = _BX(regs); _BX(regs) = (long)pgm;
	ocx = _CX(regs); _CX(regs) = (long)argv;
	odx = _DX(regs); _DX(regs) = (long)envp;
...
abi_procids(struct pt_regs *regs)
{
	...
	case 2: /* getsid */
		...
		if (get_user(pid, ((u_long *)_SP(regs))+2 + offset))
			return -EPERM;
	...
	case 5: /* setpgid */
		...
		if (get_user(pid, ((u_long *)_SP(regs))+2 + offset))
			return -EPERM;
		if (get_user(pgid, ((u_long *)_SP(regs))+3 + offset))
			return -EPERM;
...
#if defined(CONFIG_ABI_TRACE)
	//if (abi_traced(ABI_TRACE_STREAMS)) pdump(buf,error);
#endif
	return error;
}
