...
#include "../include/svr4/sigaction.h"
...
abi_sigret(int sigcall)
{
	...
	unsigned long addr, cs[4], offset;
...
down_write(&current->mm->mmap_sem);
	addr = do_mmap(NULL, 0L, PAGE_SIZE,
	PROT_READ | PROT_EXEC | PROT_WRITE, MAP_PRIVATE | MAP_EXECUTABLE | MAP_32BIT, 0);
up_write(&current->mm->mmap_sem);
	...
	cs[0]=0x0077B858; cs[1]=0x80CD0000; /*   SigReturn */
	cs[2]=0x0000ADB8; cs[3]=0x0080CD00; /* RtSigReturn */
	if (copy_to_user((char *)addr,cs,16)) return 0;
...
abi_sighold(struct pt_regs *regp)
{
	...
	fs = get_fs();
	set_fs(get_ds());
	error = SYS(rt_sigprocmask,SIG_BLOCK, &newmask,
