...
#include "../include/svr4/ioctl.h"
#include "../include/svr4/termios.h"
...
svr_to_linux_termio(int fd, int op, struct svr_termio *it)
{
	...
	fs = get_fs();
	set_fs(get_ds());
	error = SYS(ioctl,fd, TCGETA, (long)&t);
	set_fs(fs);
...
svr4_to_linux_termios(int fd, int op, struct svr4_termios *it)
{
	...
	fs = get_fs();
	set_fs(get_ds());
	error = SYS(ioctl,fd, TCGETS, (long)&t);
	set_fs(fs);
...
svr4_termiox_ioctl(int fd, u_int cmd, caddr_t data)
{
	...
	switch (cmd) {
		case 2: /* TCSETX */
			error = SYS(ioctl,fd, TCSETS, (long)&t);
			break;
		case 3: /* TCSETXW */
			error = SYS(ioctl,fd, TCSETSW, (long)&t);
			break;
		case 4: /* TCSETXF */
			error = SYS(ioctl,fd, TCSETSF, (long)&t);
			break;
	}
	set_fs(fs);
	return (error);
...
int error;
	switch (func & 0xff) {
		case 0:	 {				/* TIOCGETD */
			...
			old_fs = get_fs();
			set_fs(get_ds());
			error = SYS(ioctl,fd, TIOCGETD, (long)&ldisc);
			set_fs(old_fs);
			if (!error)
		...
		case 1: {				/* TIOCSETD */
			...
			old_fs = get_fs();
			set_fs(get_ds());
			error = SYS(ioctl,fd, TIOCSETD, (long)&ldisc);
			set_fs(old_fs);
			return error;
		}
