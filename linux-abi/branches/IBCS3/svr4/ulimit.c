...
svr4_ulimit (int cmd, int val)
{
	...
	switch (cmd) {
	case U_GETFSIZE:
		return (rlim[RLIMIT_FSIZE].rlim_cur)
		 >> ULIM_BLOCKSIZE_BITS;
	...
	case U_SETFSIZE:
		...
		if (val > rlim[RLIMIT_FSIZE].rlim_max) {
			if (!capable(CAP_SYS_RESOURCE))
				return -EPERM;
			rlim[RLIMIT_FSIZE].rlim_max = val;
		}
		rlim[RLIMIT_FSIZE].rlim_cur = val;
		return 0;

	case U_GETMEMLIM:
		return rlim[RLIMIT_DATA].rlim_cur;

	case U_GETMAXOPEN:
		return rlim[RLIMIT_NOFILE].rlim_cur;
