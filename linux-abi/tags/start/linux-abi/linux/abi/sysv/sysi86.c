/*
 *  linux/abi/svr4_common/sysi86.c
 *
...
int svr4_sysi86(struct pt_regs * regs)
{
	...
	switch (cmd) {
		case SI86FPHW: {
			 ...
			 * Unix 3.4 gives me -1...)
			 */
			get_user(np, ((unsigned long *) regs->esp) + 2);
		...
		case SETNAME: {
			...
			error = get_user(cp, ((unsigned long *) regs->esp) + 2);
			if (error)
				return error;
...
#if 1
			error = get_user(vec, ((unsigned long *) regs->esp) + 2);
			if (!error)
